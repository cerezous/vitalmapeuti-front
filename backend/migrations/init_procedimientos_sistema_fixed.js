...
async function initProcedimientosSistema() {
  try {
    ...
    // Crear procedimientos uno por uno para evitar errores de duplicados
    let creados = 0;
    for (const procedimiento of procedimientosIniciales) {
      try {
        await ProcedimientoSistema.create(procedimiento);
        creados++;
      } catch (error) {
        if (error.name === 'SequelizeUniqueConstraintError') {
        } else {
          console.error(`❌ Error al crear procedimiento "${procedimiento.nombre}":`, error.message);
        }
      }
    }
